...
function loadStatsPage() {
	...
	var losses = 0;
	var home_value;
	var opposition_value;
	for(row_counter = 2; row_counter < table.rows.length; row_counter++) {//Outer for loop iterates over each row

		home_value = parseInt(table.rows[row_counter].cells[2].innerHTML);//Read in a cells current value
		opposition_value = parseInt(table.rows[row_counter].cells[3].innerHTML);//Read in a cells current value

		if (home_value>opposition_value){
			table.rows[row_counter].cells[4].innerHTML = "CU Boulder";//Update the actual html of the cell
			wins++;
		}

		else{
			table.rows[row_counter].cells[4].innerHTML = table.rows[row_counter].cells[1].innerHTML;//Update the actual html of the cell
			losses++;
		}
	}
...
function loadPlayersPage() {
    ...
    for(var row=0; row<players.length; row++){
		selection += '<a class="dropdown-item" href="#" onclick="switchPlayers('+row+')">' +players[row].name+ '</a>';
    }
